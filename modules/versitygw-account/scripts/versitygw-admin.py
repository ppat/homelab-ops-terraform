...
def converge_bucket(bucket, owner):
    ...
    change-bucket-owner discards the bucket's ACL and calls DeleteBucketPolicy, so it
    is issued only against an owner this run actually observed. An owner that could
    not be read is never treated as a wrong owner: the two are indistinguishable here,
    ...
    if observed is None:
        ...
        if status == 409 and "AlreadyOwnedByYou" in payload:
            # It exists, this run did not create it, and the listing that would name its
            # owner did not report it. Re-homing on that basis would replace the ACL and
            # drop the policy of a bucket whose contents are unknown -- refuse instead and
            # let a human look. Unreachable if list-buckets is complete for an admin
            # credential, which is the premise this whole branch exists to not rely on.
            observed = bucket_owners().get(bucket)
            if observed is None:
                sys.exit(
                    "FAIL: bucket %s exists but list-buckets does not report it, so its "
                    "owner cannot be read; refusing to change the owner of a bucket this "
                    "run cannot see" % bucket
                )
        else:
            # The owner is assigned by the same call that creates the bucket, so a 2xx
            # here already establishes it. Nothing left to converge.
